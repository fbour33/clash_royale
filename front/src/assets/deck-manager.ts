...
const cr_cards: Card[] = [
  {
    ...
    iconUrls: {
      medium:
        "https://cdns3.royaleapi.com/cdn-cgi/image/w=150,h=180,format=auto/static/img/cards/v1-843264bf/knight.png",
      evo: "./images/knight-ev1.avif",
    },
  },
  {
    name: "Archers",
  ...
  },
  {
    ...
    iconUrls: {
      ...
      evo: "./images/barbarians-ev1.avif",
    },
  },
  {
    name: "Golem",
  ...
  },
  {
    ...
    iconUrls: {
      ...
      evo: "./images/royal-giant-ev1.avif",
    },
  },
  {
    name: "Guards",
  ...
  },
  {
    ...
    iconUrls: {
      ...
      evo: "./images/royal-recruits-ev1.avif",
    },
  },
  {
    name: "Night Witch",
  ...
  },
  {
    ...
    iconUrls: {
      ...
      evo: "./images/firecracker-ev1.avif",
    },
  },
  {
    name: "Mighty Miner",
...
class Deck {
  private static _card2Int: { [key: string]: number } = {};
  private static _int2Card: { [key: number]: Card } = {};
  private _deck: string[] = [];
  private _nbCard: number = 0;

  constructor(deckstr: string = "") {
    if (Object.keys(Deck._card2Int).length === 0) {
      Deck._card2Int = {};
      for (let i = 0; i < cr_cards.length; ++i) {
        Deck._card2Int[cr_cards[i].id.toString()] = i;
    ...
    if (deckstr !== "") {
      ...
      for (let x in tmp) this._deck.push(tmp[x]);
    }
  }
  ...
  scards(): string[] {
    ...
    for (let i = 0; i < this._nbCard; ++i) {
      ...
      if (c !== undefined) result.push(c.name);
      else {
...
let _mapN2U: { [key: string]: [string, string] } = {};

function nameToUrl(name: string, evo: boolean = false): string {
  if (Object.keys(_mapN2U).length === 0) {
    _mapN2U = {};
    for (let x in cr_cards) {
      let c = cr_cards[x];
      _mapN2U[c.name] = [c.iconUrls.medium, c.iconUrls.evo || ''];
    }
  }
  if (evo) return _mapN2U[name][1];
  else return _mapN2U[name][0];
}

let _mapN2Id: { [key: string]: number } = {};

function nameToId(name: string): number {
  if (Object.keys(_mapN2Id).length === 0) {
    _mapN2Id = {};
    for (let x in cr_cards) {
      let c = cr_cards[x];
...
let _mapId2Card: { [key: number]: Card } = {};

function idToCard(id: number): Card {
  if (Object.keys(_mapId2Card).length === 0) {
    _mapId2Card = {};
    for (let x in cr_cards) {
      let c = cr_cards[x];
      _mapId2Card[c.id] = c;
